...
async def step5(msg: Msg):
    ...
    await msg.reply(
        ...
        reply_markup=Rkm(
            keyboard=[
                [
                    "Mi interessa" if await lc(id_) else "I'm interested",  # A1
                ],
                [
                    "Non mi interessa" if await lc(id_) else "I'm not interested",  # B1
                ],
            ],
...
async def workshop_gio_gia(msg: Msg):
    ...
    await msg.reply(
        text="Questo video è una registrazione di una live di dicembre, ed è stata fatta veramente bene.\n"
             "Consiglio vivamente a tutti di guardarla almeno una volta!" if await lc(msg.chat.id)
